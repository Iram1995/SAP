  ...
  public class AddAddressForm : Form
  {
    ...
    private void txtAddress_ValueChanged(object sender, EventArgs e)
    {
      ...
      try
      {
        ...
        AddAddressForm.Item_s[] itemSArray = (AddAddressForm.Item_s[]) Utils.CopyArray((Array) ^(local = ref this.Items), (Array) new AddAddressForm.Item_s[checked (Convert.ToInt32(Decimal.Subtract(this.txtAddress.Value, 1M)) + 1)]);
        local = itemSArray;
